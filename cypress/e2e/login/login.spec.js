...
describe('Login functionality should work correctly', function () {
    ...
    it('User should be able to login with correct credential', function () {
        ...
        cy.contains(securePage.getLoggedInbanner(), securePage.getTexts().loggedInBannerText).should('exist')
    })
    ...
    it('User should be unable to login with wrong username', function () {
        ...
        cy.contains(loginPage.getFailedLoginBanner(), loginPage.getTexts().wrongUsernameText).should('exist')
    })

    it('User should be unable to login with wrong password', function () {
        loginPage.tryLogin(this.correctUsername, this.wrongPassword)
        cy.url().should('eq', `${Cypress.env('fe_base_url') + loginPage.getRoute()}`)
        cy.contains(loginPage.getFailedLoginBanner(), loginPage.getTexts().wrongPasswordText).should('exist')
    })
})
